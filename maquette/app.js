/* PCG de poche — logique de la maquette (vanilla JS, aucune dépendance). */
(() => {
  ...
  /* ---------- Modèle ---------- */
  const byNum = new Map();
  const list = PCG.accounts.map(([n, l]) => ({ n, l, nl: norm(l), c: n[0], kids: [], info: PCG.infos[n] || null, kw: (PCG.infos[n]?.k || []).map(norm) }));
  ...
  const nature = a => { for (let p = a; p; p = p.parent ? byNum.get(p.parent) : null) if (PCG.nature[p.n]) return PCG.nature[p.n]; return CLASS_NAT[a.c]; };
  ...
  function renderHome() {
    ...
    return `<div class="hint">Tapez une racine (<b>401</b>, <b>6</b>) ou un mot (<b>loyer</b>, <b>voiture</b>, <b>honoraires</b>). Le <b>i</b> explique chaque compte.</div>
      ...
      <div class="foot"><img src="logos/lagarde.png" alt="Lagarde Expertise & Conseil"><span>Usage interne<br>Maquette v0.1</span><img src="logos/ac.png" alt="L'Agent Comptable"></div>`;
  }
  ...
  /* ---------- Fiche compte ---------- */
  const link = a => `<button class="link" data-open="${a.n}" style="--cc:var(--c${a.c})"><span class="num mono">${a.n}</span><span class="lbl">${esc(a.l)}</span></button>`;
  function openSheet(n, push = true) {
    ...
    const nc = (info?.nc || []).map(x => Array.isArray(x) ? x : [x, ""]).map(([m, why]) => byNum.get(m)).filter(Boolean);
    const ncHtml = (info?.nc || []).map(x => Array.isArray(x) ? x : [x, ""]).map(([m, why]) => { const b = byNum.get(m); return b ? `<button class="link" data-open="${b.n}" style="--cc:var(--c${b.c})"><span class="num mono">${b.n}</span><span class="lbl">${esc(b.l)}${why ? `<small>${esc(why)}</small>` : ""}</span></button>` : ""; }).join("");
    ...
    sheet.innerHTML = `<div class="handle"></div>
      ...
      <div class="sheet-body">
        ...
        <div class="block"><h4>En clair</h4>${info?.i ? `<p>${esc(info.i)}</p>` : `<p class="muted">Explication à rédiger. La maquette n'en contient que pour un échantillon de comptes ; l'application finale en aura une pour chacun.</p>`}</div>
        ${ncHtml ? `<div class="block"><h4>À ne pas confondre avec</h4><div class="links">${ncHtml}</div></div>` : ""}
        ${rel.length ? `<div class="block"><h4>Comptes liés</h4><div class="links">${rel.map(link).join("")}</div></div>` : ""}
        ${a.kids.length ? `<div class="block"><h4>Sous-comptes</h4><div class="links">${a.kids.map(link).join("")}</div></div>` : ""}
        ${info?.k?.length ? `<div class="block"><h4>Mots-clés</h4><div class="kw">${info.k.map(x => `<span>${esc(x)}</span>`).join("")}</div></div>` : ""}
        <div class="sheet-acts">
          <button data-act="fav" class="${fav ? "on" : ""}">${ICON.star} ${fav ? "Favori" : "Favori"}</button>
